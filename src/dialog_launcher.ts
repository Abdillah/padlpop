...
export class Launcher extends search.Search {
    options: Array<launchers.SearchOption>
    desktop_apps: Array<[string, AppInfo]>
    last_plugin: null | launchers.Plugin.Source
    mode: number;

    constructor(ext: Ext) {
        let service = new launchers.LauncherService()
        ...
        let search = (pattern: string): Array<launchers.SearchOption> | null => {
            ...
            let res = service.query(pattern)
            if (res) {
                const [plugin, response] = res
                this.last_plugin = plugin;
                if (response.kind === "queried") {
                    for (const selection of response.selections) {
                        let icon = selection.icon
                            ? { name: selection.icon }
                            : { gicon: Gio.content_type_get_icon(selection.content_type) }

                        this.options.push(new launchers.SearchOption(
                            selection.name,
        ...
        let apply = (index: number): boolean => {
            ...
            if ("window" in option) {
                ...
                global.log(`submitting option to plugin`)
                const { plugin, id } = option
                launchers.Plugin.submit(plugin, id)
        ...
        let complete = () => {
            if (this.last_plugin) {
                launchers.Plugin.complete(this.last_plugin)
                const res = launchers.Plugin.listen(this.last_plugin)
                if (res && res.kind === "fill") {
...
function window_selection(ext: Ext, window: ShellWindow, icon_size: number): launchers.SearchOption {
    ...
    return new launchers.SearchOption(
        name,
        null,
